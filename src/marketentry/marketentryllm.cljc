(ns marketentry.marketentryllm
  ...
  the PAK public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-pk-entity?=" (:has-pk-entity? e)
                        " ntn-verified?=" (:ntn-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-pk-entity? e))
                              (:has-pk-entity? e))
                          (or (not (:requires-ntn? e))
                              (:ntn-verified? e)))
                   0.9 0.3)}))
