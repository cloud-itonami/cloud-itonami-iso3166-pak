(ns marketentry.store
  "SSoT for the PAK market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems PK" :portal "e-PADS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-pk-entity? true :has-pk-entity? true
             :requires-ntn? true :ntn-verified? true
             :drafted? false :submitted? false
             :jurisdiction "PAK" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "e-PADS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-pk-entity? true :has-pk-entity? true
             :requires-ntn? true :ntn-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "e-PADS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "PAK" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "e-PADS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-pk-entity? true :has-pk-entity? false
             :requires-ntn? true :ntn-verified? true
             :drafted? false :submitted? false
             :jurisdiction "PAK" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "e-PADS/PPRA"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-pk-entity? true :has-pk-entity? true
             :requires-ntn? true :ntn-verified? false
             :drafted? false :submitted? false
             :jurisdiction "PAK" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "e-PADS/PPRA"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-pk-entity? has-pk-entity?
                               requires-ntn? ntn-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-pk-entity?)  (assoc :engagement/requires-pk-entity? requires-pk-entity?)
    (some? has-pk-entity?)       (assoc :engagement/has-pk-entity? has-pk-entity?)
    (some? requires-ntn?)    (assoc :engagement/requires-ntn? requires-ntn?)
    (some? ntn-verified?)    (assoc :engagement/ntn-verified? ntn-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-pk-entity? :engagement/has-pk-entity?
   :engagement/requires-ntn? :engagement/ntn-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-pk-entity? (boolean (:engagement/requires-pk-entity? m))
     :has-pk-entity? (boolean (:engagement/has-pk-entity? m))
     :requires-ntn? (boolean (:engagement/requires-ntn? m))
     :ntn-verified? (boolean (:engagement/ntn-verified? m))
