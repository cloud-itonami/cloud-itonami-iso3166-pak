(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Pakistan-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Pakistan-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-pk-entity?
                                       true` (near-universal for Pakistan
                                       ...
                                       Grounded in Pakistani legal entity
                                       (SECP/NTN) typically required for
                                       federal awards (contracting authorities
                                       / PPRA).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(NTN/統一資格/e-PADS/PPRA登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- pk-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-pk-entity? true`, INDEPENDENTLY verify
  `:has-pk-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-pk-entity?` ground truth (most Pakistan public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-pk-entity? e))
                 (not (true? (:has-pk-entity? e))))
        [{:rule :pk-entity-missing
          :detail (str subject " はパキスタン居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- ntn-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ntn? true`, INDEPENDENTLY check
  `:ntn-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ntn? e))
                 (not (true? (:ntn-verified? e))))
        [{:rule :ntn-unverified
          :detail (str subject " はNTN確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (pk-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (ntn-unverified-violations request st)
